...
class PreregCart:
    ...
    def purchaser(self):
        ...
        If attendee accounts are enabled we always use the account for the purchaser ID and receipt email,
        so this returns None.
        """
        from uber.models import Session
        from uber.utils import get_age_from_birthday

        if not self.models or c.ATTENDEE_ACCOUNTS_ENABLED:
            return

        maybe_purchasers = []

        for model in self.models:
            if model.birthdate and get_age_from_birthday(model.birthdate, c.NOW_OR_AT_CON) >= 18:
...
class StripeRequestMixin:
    ...
    def send_refund_request(self, session, amount, charge_id, intent_id):
        try:
...
class AuthNetRequestMixin:
    ...
    def send_refund_request(self, session, amount, charge_id, intent_id):
        ...
        if self.response.transactionStatus == "capturedPendingSettlement":
            ...
            error = self.send_authorizenet_txn(session, txn_type=c.VOID, txn_id=charge_id)
            ...
            error = self.send_authorizenet_txn(session, txn_type=c.REFUND, amount=amount, cc_num=cc_num,
    ...
    def send_authorizenet_txn(self, session, txn_type=c.AUTHCAPTURE, **params):
            ...
            if response.messages.resultCode == "Ok":
                ...
                if hasattr(response.transactionResponse, 'messages') is True:
                    ...
                    if txn_type in [c.AUTHCAPTURE, c.CAPTURE]:
                        ReceiptManager.mark_paid_from_ids(session, params.get('intent_id'), auth_txn_id,
...
class TransactionRequest(AuthNetRequestMixin if c.AUTHORIZENET_LOGIN_ID else StripeRequestMixin):
    def __init__(self, session, receipt=None, receipt_email='', description='', amount=0,
                 method=c.STRIPE, customer_id=None, **kwargs):
        self.session = session
        self.amount = int(amount)

        # The account should be passed to this function if a transaction is handled by an admin
        # OR to force using the receipt_email by passing account=None
        if 'account' in kwargs:
            self.account = kwargs['account']
        elif c.ATTENDEE_ACCOUNTS_ENABLED:
            self.account = session.current_attendee_account()
        else:
            self.account = None
        
        if self.account:
            self.receipt_email = self.account.email
        else:
...
class RefundRequest(TransactionRequest):
    def __init__(self, session, txns, amount=0, skip_errors=False, who='', **kwargs):
        ...
        if txns[0].method == c.SQUARE and c.SPIN_TERMINAL_AUTH_KEY:
            ...
            self.spin_request = SpinTerminalRequest(session, amount=self.amount)
        else:
    ...
    def validate_or_add_txn(self, txn):
        ...
        if not txn.charge_id:
            charge_id = txn.check_paid_from_stripe(self.session)
            if not charge_id:
    ...
    def process_txn_refund(self, department=None):
        ...
        message = self.send_refund_request(self.session, self.amount, self.charge_id, self.txns[0].intent_id)
        if message:
            return message
...
class SpinTerminalRequest(TransactionRequest):
    def __init__(self, session, terminal_id='', amount=0, capture_signature=None, tracker=None, spin_payment_type="Credit",
        ...
        super().__init__(session, amount=amount, **kwargs)
    ...
    def process_successful_sale(self, session, response_json, intent_id):
        ...
        ReceiptManager.mark_paid_from_ids(session, intent_id, self.terminal_id + "-" + intent_id)
...
class ReceiptManager:
    ...
    def get_purchaser_id(cls, receipt=None, model=None):
        ...
        if isinstance(model, (Attendee, Group)):
            return model.purchaser_id
        else:
            purchaser = getattr(model, 'attendee', None)
            return purchaser.purchaser_id if purchaser else None
    ...
    @classmethod
    def create_new_receipt(cls, model, who='', create_model=False, purchaser_id=None, include_discounts=True):
        """
        ...
        """
        from uber.models import Attendee, AdminAccount, ModelReceipt, ReceiptItem, Group
        ...
        if isinstance(model, Attendee) and include_discounts:
            age_discount = ReceiptManager.check_age_discount(model, receipt, who)
            promo_code_discounts = ReceiptManager.check_promo_code_discounts(model, receipt, who)
            if receipt:
                if age_discount:
                    receipt_items.append(age_discount)
                if promo_code_discounts:
                    receipt_items.extend(promo_code_discounts)
            else:
                if age_discount:
                    receipt_items.append((age_discount.discount_str, age_discount.amount, 1))
                for discount in promo_code_discounts:
                    receipt_items.append((discount.discount_str, discount.amount, 1))
    ...
    @classmethod
    def auto_update_receipt(self, session, model, receipt, params, who=''):
        ...
        if new_promo_code or ('promo_code_code' not in params and old_promo_code):
            session.add_promo_code_to_attendee(new_model, new_promo_code or old_promo_code)
        ...
        if isinstance(model, Attendee):
            ReceiptManager.update_receipt_discounts(session, model, new_model, receipt, who=who)

        return receipt_items
    
    @classmethod
    def update_receipt_discounts(cls, session, attendee, new_attendee, receipt, who=''):
        if not new_attendee and attendee.age_discount or attendee.age_discount != new_attendee.age_discount:
            existing_age_discounts = [discount for discount in receipt.receipt_discounts if 'Age Discount' in discount.desc]
            for existing_discount in existing_age_discounts:
                session.delete(existing_discount)
            age_discount = ReceiptManager.check_age_discount(new_attendee or attendee, receipt, who)
            if age_discount:
                session.add(age_discount)

        if not new_attendee and attendee.promo_code or attendee.promo_code != new_attendee.promo_code:
            existing_promo_discounts = [discount for discount in receipt.receipt_discounts if 'Promo Code' in discount.desc]
            for existing_discount in existing_promo_discounts:
                session.delete(existing_discount)
    ...
    @classmethod
    def check_age_discount(cls, attendee, receipt, who):
        from uber.models import ReceiptDiscount
        if not attendee.age_discount:
            return
        
        age_discount = ReceiptDiscount(receipt_id=receipt.id if receipt else None, who=who, desc=f"Age Discount")
        if attendee.age_discount < 1:
            age_discount.discount = attendee.age_discount * 100
            age_discount.discount_type = c.PERCENT_DISCOUNT
        elif attendee.age_discount >= 999:
            age_discount.discount = None  # Free badge
        else:
            age_discount.discount = attendee.age_discount
            age_discount.discount_type = c.FIXED_DISCOUNT
        age_discount.set_discount(attendee)

        return age_discount
    
    @classmethod
    def check_promo_code_discounts(cls, attendee, receipt, who):
        from uber.models import ReceiptDiscount
        if not attendee.promo_code:
            return []
        ...
        if attendee.promo_code:
            ...
            for discount_item in promo_code.discount_on_ints:
                ...
                new_discount = ReceiptDiscount(
                    ...
                    department=promo_code.department,
                    discount=promo_code.discount,
                    discount_type=promo_code.discount_type,
                    discount_on=int(discount_item),
                    who=who,
                    desc=f"Promo Code {promo_code.code}"
                )
                new_discount.set_discount(attendee)
                promo_discounts.append(new_discount)

        return promo_discounts

    @staticmethod
    def mark_paid_from_stripe_intent(session, payment_intent):
        ...
        return ReceiptManager.mark_paid_from_ids(session, payment_intent.id, payment_intent.latest_charge)

    @staticmethod
    def mark_paid_from_ids(session, intent_id, charge_id, put_on_hold=False):
